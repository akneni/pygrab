...
impl SessionRs {
    ...
    pub fn get(&mut self, url: String) -> PyResult<HttpResponse> {
        ...
        match res {
            Ok(x) => {
                self.num_req += 1;
                Ok(HttpResponse::from_reqwest(x))
            }
            Err(x) => { Err(PyException::new_err(format!("Error: {}", x))) }
        }
    }

    pub fn get_async (&mut self, urls: Vec<String>, thread_limit: u32, warn_status: Option<bool>) -> HashMap<String, HttpResponse> {
        ...
        while ind < urls.len() {
            ...
            for (url, response) in rx {
                res.insert(url, HttpResponse::from_reqwest(response));
                self.num_req += 1;
            }
        }
        res
    ...
    pub fn download_async (&mut self, urls: Vec<String>, filenames: Vec<String>, thread_limit: u32, warn_status: Option<bool>) {
        let client_copy = Arc::new(self.client.clone());
        let mut url_iter = urls.iter();
        ...
        while ind < urls.len() {
            ...
            for _ in 0..thread_limit {
                ...
                handles.push(thread::spawn(move || {
                    let res = Self::download_async_helper(url.clone(), filename, client_cloned);
                    if let Err(_) = res {
    ...
    // ↓↓ hey rust devs? function overloading or default parameters would be amazing :D ↓↓
    pub fn post(&mut self,url: String, data:String) -> PyResult<HttpResponse> {
    ...
    pub fn post_async(
        ...
        while ind < urls.len() {
            ...
            for _ in 0..thread_limit {
                ...
                if url.is_none() { break }
                let url = url.unwrap().clone();
                let text = body_iter.next().unwrap().clone();
                
                ind += 1;
                thread::spawn(move || {
                    let res = client_cloned
                        .post(&url)
    ...
    pub fn post_bytes(&mut self,url: String, data:Vec<u8>) -> PyResult<HttpResponse> {
        let res = self.client.post(url)
            .body(data)
            .send();
        match res {
...
impl SessionRs {
    fn download_async_helper(url: String, filename: String, client: Arc<Client>) -> Result<(), ()> {
